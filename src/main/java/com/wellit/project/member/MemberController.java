...
public class MemberController {
	...
	public String getMypage(Model model) {
		...
		System.out.println("12312123123123132123321" + authentication);
	...
	/*
	 * @GetMapping("/mypage") public String getMypage(Model model) { Authentication
	 * authentication = SecurityContextHolder.getContext().getAuthentication();
	 * 
	 * if (authentication != null) { Object principal =
	 * authentication.getPrincipal();
	 * 
	 * if (principal instanceof UserDetails) { // 일반 로그인 UserDetails userDetails =
	 * (UserDetails) principal; String memberId = userDetails.getUsername(); Member
	 * member = memberService.getMember(memberId);
	 * 
	 * DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
	 * String formattedRegDate = member.getMemberRegDate().format(formatter);
	 * 
	 * model.addAttribute("member", member); model.addAttribute("formattedRegDate",
	 * formattedRegDate);
	 * 
	 * } else if (principal instanceof OAuth2User) { // OAuth2 로그인 (예: Kakao)
	 * OAuth2User oAuth2User = (OAuth2User) principal; String memberId = (String)
	 * oAuth2User.getAttributes().get("id"); // 예시로 ID를 가져오는 코드
	 * 
	 * Member member = memberService.getMember(memberId);
	 * 
	 * DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
	 ...
	 * 
	 * model.addAttribute("member", member); model.addAttribute("formattedRegDate",
	 * formattedRegDate); } } return "member/mypage"; }
	 */
	...
	public String getEnterPassword(Model model) {
		...
		if (authentication != null && authentication.isAuthenticated()) {
			...
			if (principal instanceof String) {
				...
				model.addAttribute("member", member);
				return "member/enter_password"; // 경로 수정
			}
			// UserDetails를 사용하는 경우
	...
	public String updateProfile(@Valid MemberUpdateForm memberUpdateForm, BindingResult bindingResult,
		...
		if (authentication != null && authentication.isAuthenticated()) {
			...
			model.addAttribute("updateMessage" , "문제 생김");
			return "redirect:/member/login";
		}
	...
	public String getDeleteMember(Model model) {
		...
		if (authentication != null && authentication.isAuthenticated()) {
			...
			if (principal instanceof String) {
				...
				model.addAttribute("member", member);
				return "member/delete_password"; // 경로 수정
			}
			// UserDetails를 사용하는 경우
			else if (principal instanceof UserDetails) {
				UserDetails userDetails = (UserDetails) principal;
